...
rectangle:
	...
	sll	$a1, $a1, 2
	sll	$a2, $a2, 8 # scale y values to bytes (512*4/8 bytes per display row)
	sll	$a3, $a3, 8
	...
	li	$t4, 0x100 # bytes per display row
...
draw_game:
	...
	li	$t0, 0xe3de8f 	#amarelo apagado
	li	$a0, 15		#cord x
	li	$a1, 9		#largura
	li	$a2, 11		#CORD Y
	li	$a3, 9		#altura
...
_msg1: .asciiz "\n!! Ocorreu uma exceção !!\n Codigo de erro: "
_msg_exc4:	.asciiz "\nException cause: Adress error caused by load or fetch instruction\n"
_msg_exc5:	.asciiz "\nException cause: Adress error caused by store\n"
_msg_exc6:	.asciiz "\nException cause: Bus error on instruction fetch\n"
_msg_exc7:	.asciiz "\nException cause: Bus error on data or store\n"
_msg_exc8:	.asciiz "\nException cause: Syscall instruction\n"
_msg_exc9:	.asciiz "\nException cause: Break instruction\n"
_msg_exc10:	.asciiz "\nException cause: Reserved instruction\n"
_msg_exc12:	.asciiz "\nException cause: Arithmetic overflow\n"
_msg_exc13:	.asciiz "\nException cause: Trap\n"
_msg_exc15:	.asciiz "\nException cause: Floating point\n "
...
la	$k0, _regs		# carrega local para salvar o estado de registradores que queremos usar
...
beqz	$a0, interrupt_handler	# se $a0 for 0, foi interrupção e não exceção
	...
	addi $a1, $a1, 4
	mtc0 $a1, $14
	beq $a0, 4, _msg_excp4_code
	beq $a0, 5, _msg_excp5_code
	beq $a0, 6, _msg_excp6_code
	beq $a0, 7, _msg_excp7_code
	beq $a0, 8, _msg_excp8_code
	beq $a0, 9, _msg_excp9_code
	beq $a0, 10, _msg_excp10_code
	beq $a0, 12, _msg_excp12_code
	beq $a0, 13, _msg_excp13_code
	beq $a0, 15, _msg_excp15_code
...
exit_kernel:
la	$k0, _regs		# recuperação dos registradores usados
lw	$a0, 0($k0)
lw	$a1, 4($k0)
lw	$v0, 8($k0)
move	$at, $k1
